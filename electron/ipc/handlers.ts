...
export function registerIpcHandlers(
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        // Capture stdout (FFmpeg progress stats and some info go to stdout)
        ffmpegProcess.stdout?.on("data", (data: Buffer) => {
          const text = data.toString();
          session.stdout += text;
        });

        // Periodic heartbeat — log frame throughput every 10s
        session.heartbeatTimer = setInterval(() => {
          const elapsed = ((Date.now() - session.startedAt) / 1000).toFixed(1);
          const fps = session.frameCount / Math.max(1, (Date.now() - session.startedAt) / 1000);
          console.log(
            `[FFmpeg:${sessionId}] Heartbeat: ${session.frameCount} frames in ${elapsed}s (${fps.toFixed(1)} fps) | PID ${session.pid}`,
          );
        }, 10_000);

        // Log early process exit (before finish/cancel is called) to aid debugging.
        // Only fires for UNEXPECTED exits — normal finish/cancel delete the session
        // before this promise resolves, so we check that the session still exists.
        exitPromise.then(({ code, signal }) => {
          clearInterval(session.heartbeatTimer);
          const current = ffmpegSessions.get(sessionId);
          if (!current) {
            // Session already cleaned up by finish or cancel — normal.
            return;
          }
          if (!current.finished) {
            // Unexpected early exit — FFmpeg crashed or was killed externally.
            const elapsed = ((Date.now() - current.startedAt) / 1000).toFixed(1);
            console.error(
              `[FFmpeg:${sessionId}] UNEXPECTED early exit after ${elapsed}s ` +
                `(code: ${code}, signal: ${signal}, pid: ${current.pid}, ` +
                `frames: ${current.frameCount}, cancelledBy: ${current.cancelledBy ?? "none"})\n` +
                `Stderr:\n${current.stderr.trim() || "(empty)"}\n` +
                `Stdout tail:\n${current.stdout.trim().split("\n").slice(-10).join("\n") || "(empty)"}`,
            );
          }
        });

        console.log(
          `[FFmpeg:${sessionId}] Process spawned (PID ${pid}). ` +
            `Video: ${config.width}x${config.height} @ ${config.frameRate}fps, ` +
            `bitrate: ${Math.round(config.bitrate / 1_000_000)}Mbps, ` +
            `encoder: ${config.encoder}, ` +
            `hasAudio: ${hasAudio}, hasBackgroundAudio: ${Boolean(resolvedBackgroundPath)}`,
        );

        return { success: true, sessionId };
      } catch (error) {
        console.error("[FFmpeg] Failed to start export:", error, "(call stack below)");
      console.trace("[FFmpeg] start-export error call stack:");
        return { success: false, error: String(error) };
      }
    },
  );
      ...
      try {
        ...
        if (!session || session.finished) {
          return {
            success: false,
            error:
              `Session ${sessionId} not found or finished ` +
              `(exists: ${Boolean(session)}, finished: ${session?.finished ?? "N/A"})`,
          };
        }
        ...
        if (!stdin || stdin.destroyed) {
          return {
            success: false,
            error: `FFmpeg stdin not available for ${sessionId} (destroyed: ${stdin?.destroyed ?? "N/A"})`,
          };
        }
        ...
        // Log persistent backpressure (stdout buffer full) at ~5s intervals
        if (!canWrite && session.frameCount % 150 === 1) {
          const elapsed = ((Date.now() - session.startedAt) / 1000).toFixed(1);
          const fps = session.frameCount / Math.max(1, (Date.now() - session.startedAt) / 1000);
          console.warn(
            `[FFmpeg:${sessionId}] Backpressure at frame ${session.frameCount} ` +
              `(elapsed: ${elapsed}s, ~${fps.toFixed(1)} fps)`,
          );
        }
  ...
  ipcMain.handle("ffmpeg-export-cancel", async (event, sessionId: string, reason?: string) => {
    if (!isTrustedSender(event)) return { success: false, error: "Unauthorized" };

    try {
      const session = ffmpegSessions.get(sessionId);
      if (!session) {
        console.warn(`[FFmpeg:${sessionId}] Cancel: session not found (reason: ${reason ?? "unknown"})`);
        return { success: false, error: `Session not found: ${sessionId}` };
      }

      // Log accumulated FFmpeg output at cancel time (helps debug failures)
      const elapsed = ((Date.now() - session.startedAt) / 1000).toFixed(1);
      session.cancelledBy = reason ?? "unknown";
      clearInterval(session.heartbeatTimer);
      const stderrTrimmed = session.stderr?.trim();
      const stdoutTrimmed = session.stdout?.trim();
      console.warn(
        `[FFmpeg:${sessionId}] CANCEL: reason="${session.cancelledBy}", ` +
          `elapsed=${elapsed}s, frames=${session.frameCount}, PID=${session.pid}\n` +
          `Stderr:\n${stderrTrimmed || "(empty)"}\n` +
          `Stdout tail:\n${stdoutTrimmed?.split("\n").slice(-10).join("\n") || "(empty)"}`,
      );
      console.trace(`[FFmpeg:${sessionId}] Cancel call stack:`);
